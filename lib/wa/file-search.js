...
export function needsKnowledgeBase(message) {
  ...
  const keywords = [
    ...
    'descuento',
    // Objeciones - precio
    'cara', 'caro', 'muy caro', 'muy cara',
    'está caro', 'esta caro', 'está cara', 'esta cara',
    'es caro', 'es cara',
    'costoso', 'costosa',
    // Objeciones - tiempo
    'lo voy a pensar', 'voy a pensar', 'déjame pensarlo', 'dejame pensarlo',
    // Objeciones - dinero
    'no tengo dinero', 'no tengo plata', 'no me alcanza',
    // Objeciones - necesidad
    'ya tengo', 'no necesito', 'no me interesa',
    // Manejo de objeciones general
    'objeción', 'objecion', 'cliente dice', 'cliente pregunta',
    'clienta dice', 'clienta pregunta',
    'cómo manejo', 'como manejo', 'qué hago', 'que hago',
    'cómo respondo', 'como respondo', 'qué le digo', 'que le digo',
    'cómo convenzo', 'como convenzo',
    // Políticas
  ...
  // Log para debugging
  if (found) {
    const matchedKeyword = keywords.find(k => messageLower.includes(k));
    console.log(`🔍 KB trigger detected: "${matchedKeyword}" in message`);
  }
