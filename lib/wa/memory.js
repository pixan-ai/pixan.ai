 ...
 * Now includes knowledge base in system prompt
 */

import { db } from './redis.js';
import { chat } from './ai.js';
import { MEMORY, DEFAULT_SYSTEM_PROMPT } from './config.js';
import { getKnowledgeContext } from './file-search.js';
...
// Build messages array for AI call - NOW INCLUDES KNOWLEDGE BASE
export const buildMessages = async (userId, userContent, modelId) => {
  const [systemPrompt, memory, summary, knowledge] = await Promise.all([
    getSystemPrompt(),
    getMemory(userId),
  ...
  if (knowledge.context) {
    fullSystemPrompt += knowledge.context;
  }
  ...
  return {
    messages,
    hasKnowledge: knowledge.documentCount > 0,
    knowledgeDocCount: knowledge.documentCount
  };
};
